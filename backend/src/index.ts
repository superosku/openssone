...
app.get('/games', async (req: Request, res: Response) => {
  ...
  const gamesWithId = games.map((gState) => {
    return {...gState, id: gState._id};
  });
...
app.get('/games/:gameId', async (req: Request, res: Response) => {
  ...
  const currentPlayer = game.players.filter((player) => player.joinSlug === joinSlug)[0];
...
app.ws('/messages/:gameId', async (webSocket: ws, req: Request) => {
  const gameId = req.params.gameId;
  ...
  const authHeaderString = (req.headers['sec-websocket-protocol'] || '') as string;
  const joinSlug = authHeaderString.split(' ')[1];
  const currentPlayer = origGame.players.filter((player) => player.joinSlug === joinSlug)[0];
  ...
  webSocket.on('message', async (msg) => {
    ...
    if (data.type === 'end-turn') {
      const curPlayerIndex = currentGame.players.findIndex((player) => player.id === currentPlayer.id);
