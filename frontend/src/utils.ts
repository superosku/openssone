...
const pieceImageCache: { [key: string]: string } = {};

export const getImageDataUrl = (piece: Piece): string => {
  ...
  // Full castle
  if (piece.sideTypes.every((st) => st === PieceSideType.castle)) {
  ...
  for (let i = 0; i < 4; i++) {
    ...
    if (sideType === PieceSideType.road || sideType === PieceSideType.river) {
      ...
      for (let j = 0; j < colorsAndWidths.length; j++) {
        ...
        if (
          ...
          ctx.stroke();
            ...
            sideType === PieceSideType.river && (
              piece.sideTypes.some((st) => st === PieceSideType.road) ||
              piece.extraInfo === PieceExtraInfo.monastery
            )
          ) {
            // Water should bend to avoid roads or monastery
            // Needed to make water go around on the same side on both directions
          const thing = Math.floor(i / 2) % 2 === 0 ? 15 : -15;
        ...
        const roadOrRiverCount = piece.sideTypes.reduce((acc: number, st) => acc + (st === sideType ? 1 : 0), 0);
